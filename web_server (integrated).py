...
def send_complete(request_id):
    for client in clients:
        #client.send(str(request_id).encode("utf-8"), True)
        cherrypy.log("Sent data '" + str(request_id) + "' to " +str(client._peer_address[0]))

class status_socket(WebSocket):
    global clients
    
    def received_message(self, message):
        print(message.is_binary)
        self.send(message.data, message.is_binary)
        cherrypy.log("Received data '" + message.data.decode("utf-8") + "' from " + str(self._peer_address[0]))

    def opened(self):
        cherrypy.log("Socket opened from " + str(self._peer_address[0]))
        clients.append(self)
        self.send("5", False)
        print("Sent a five")
    
    def closed(self, code, reason=""):
        cherrypy.log("Socket closed to " + str(self._peer_address[0]))
        clients.remove(self)
...
if __name__ == "__main__":
    ...
    server_config = {
                        "/": {
                            "log.access_file": root_data + "logs/serverlog.log",
                            "log.error_file": "",
                        ...
                        "/status": {
                            ...
                            "tools.websocket.handler_cls": status_socket
                        },
                        "/static": {
                            "tools.staticdir.on": True,
                            "tools.staticdir.dir": root_main + "static"
                        },
                        "/favicon.ico": {
                            "tools.staticfile.on": True,
                            "tools.staticfile.filename": root_main + "static/img/favicon.ico"
                        }
                    }
    cherrypy.quickstart(main_server(), "/", server_config)
